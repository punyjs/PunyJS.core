...
* @factory
*/
function _ElementHelper(createElement, createTextNode, getElementById, getElementsByTagName, querySelector, createElementNs, is_element, is_array, is_nill, is_integer) {
    ...
    */
    function addToTarget(el, target, insert) {
        if (!!target) {
            ...
            if (is_nill(insert) || !target.children[insert]) {
                target.appendChild(el);
            }
                //if the insert value is an integer then insert before
            else if (Number.is_integer(insert)) {
        ...
        */
        , "find": {
            ...
            , "value": function find(selector) {
                ...
                //check for erroneous el argument
                else if (!!id && !is_element(selector)) {
                    throw new Error("The id argument must be a string or an HTMLElement");
                }
                else {
